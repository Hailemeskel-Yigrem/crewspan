"""Workflow helpers for Webhook — side effects, guards, and audit notes."""
...
class WebhookWorkflow:
    """Coordinates lifecycle events for webhook with validation and logging."""

    ALLOWED_STATUSES = ("draft", "pending", "active", "in_progress", "blocked", "completed", "cancelled")
    DOMAIN_ACTIONS = ("trigger_test", "rotate_secret", "disable_on_failures")

    def __init__(self, *, strict: bool = True) -> None:
        self._strict = strict

    def plan(self, entity: dict[str, Any], action: str, **context: Any) -> WebhookWorkflowResult:
        ...
        logger.info(
            "webhook.workflow.plan",
            action=action,
            entity_id=entity_id,
            from_status=current,
            to_status=nxt,
        )
        return WebhookWorkflowResult(
            action=action,
            entity_id=entity_id,
    ...
    def apply(self, entity: dict[str, Any], action: str, **context: Any) -> WebhookWorkflowResult:
        result = self.plan(entity, action, **context)
        entity["status"] = result.next_status
        entity["updated_at"] = result.executed_at.isoformat()
        logger.info("webhook.workflow.apply", entity_id=result.entity_id, action=action)
        return result
    ...
    def _reject(self, message: str) -> None:
        ...
        logger.warning("webhook.workflow.soft_fail", message=message)
