"""Workflow helpers for SlaBreach — side effects, guards, and audit notes."""
...
class SlaBreachWorkflow:
    """Coordinates lifecycle events for sla_breach with validation and logging."""

    ALLOWED_STATUSES = ("draft", "pending", "active", "in_progress", "blocked", "completed", "cancelled")
    DOMAIN_ACTIONS = ("acknowledge", "escalate")

    def __init__(self, *, strict: bool = True) -> None:
        self._strict = strict

    def plan(self, entity: dict[str, Any], action: str, **context: Any) -> SlaBreachWorkflowResult:
        ...
        logger.info(
            "sla_breach.workflow.plan",
            action=action,
            entity_id=entity_id,
            from_status=current,
            to_status=nxt,
        )
        return SlaBreachWorkflowResult(
            action=action,
            entity_id=entity_id,
    ...
    def apply(self, entity: dict[str, Any], action: str, **context: Any) -> SlaBreachWorkflowResult:
        result = self.plan(entity, action, **context)
        entity["status"] = result.next_status
        entity["updated_at"] = result.executed_at.isoformat()
        logger.info("sla_breach.workflow.apply", entity_id=result.entity_id, action=action)
        return result
    ...
    def _reject(self, message: str) -> None:
        ...
        logger.warning("sla_breach.workflow.soft_fail", message=message)
