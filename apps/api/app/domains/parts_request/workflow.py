"""Workflow helpers for PartsRequest — side effects, guards, and audit notes."""
...
class PartsRequestWorkflowResult:
    action: str
    entity_id: str
    previous_status: str
    next_status: str
...
class PartsRequestWorkflow:
    """Coordinates lifecycle events for parts_request with validation and logging."""

    ALLOWED_STATUSES = ("draft", "pending", "active", "in_progress", "blocked", "completed", "cancelled")
    DOMAIN_ACTIONS = ("approve", "fulfill", "reject")

    def __init__(self, *, strict: bool = True) -> None:
        self._strict = strict

    def plan(self, entity: dict[str, Any], action: str, **context: Any) -> PartsRequestWorkflowResult:
        ...
        logger.info(
            "parts_request.workflow.plan",
            action=action,
            entity_id=entity_id,
            from_status=current,
            to_status=nxt,
        )
        return PartsRequestWorkflowResult(
            action=action,
            entity_id=entity_id,
    ...
    def apply(self, entity: dict[str, Any], action: str, **context: Any) -> PartsRequestWorkflowResult:
        result = self.plan(entity, action, **context)
        entity["status"] = result.next_status
        entity["updated_at"] = result.executed_at.isoformat()
        logger.info("parts_request.workflow.apply", entity_id=result.entity_id, action=action)
        return result
    ...
    def _reject(self, message: str) -> None:
        ...
        logger.warning("parts_request.workflow.soft_fail", message=message)
