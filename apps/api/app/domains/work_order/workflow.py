"""Workflow helpers for WorkOrder — side effects, guards, and audit notes."""
...
class WorkOrderWorkflow:
    """Coordinates lifecycle events for work_order with validation and logging."""

    ALLOWED_STATUSES = ("draft", "pending", "active", "in_progress", "blocked", "completed", "cancelled")
    DOMAIN_ACTIONS = ("submit", "assign_technician", "start", "complete", "cancel")

    def __init__(self, *, strict: bool = True) -> None:
        self._strict = strict

    def plan(self, entity: dict[str, Any], action: str, **context: Any) -> WorkOrderWorkflowResult:
    ...
    def apply(self, entity: dict[str, Any], action: str, **context: Any) -> WorkOrderWorkflowResult:
        result = self.plan(entity, action, **context)
        entity["status"] = result.next_status
        entity["updated_at"] = result.executed_at.isoformat()
        logger.info("work_order.workflow.apply", entity_id=result.entity_id, action=action)
        return result
    ...
    def _reject(self, message: str) -> None:
        ...
        logger.warning("work_order.workflow.soft_fail", message=message)
