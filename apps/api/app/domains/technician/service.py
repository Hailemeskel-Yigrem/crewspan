"""Business logic for Technician."""

from __future__ import annotations

from datetime import date
from decimal import Decimal
from typing import Any
from uuid import UUID

import structlog

from app.domains.technician.exceptions import (
    TechnicianConflictError,
    TechnicianNotFoundError,
    TechnicianValidationError,
)
from app.domains.technician.models import Technician
from app.domains.technician.repository import TechnicianRepository
from app.domains.technician.schemas import TechnicianCreate, TechnicianRead, TechnicianUpdate

logger = structlog.get_logger(__name__)


class TechnicianService:
    """Orchestrates technician use cases with validation, transitions, and auditing."""

    def __init__(self, repository: TechnicianRepository) -> None:
        self._repo = repository

    _STATUS_TRANSITIONS: dict[str, set[str]] = {
        "draft": {"submitted", "cancelled", "active", "pending"},
        "pending": {"approved", "rejected", "cancelled", "active"},
        "submitted": {"in_progress", "assigned", "cancelled"},
        "assigned": {"in_progress", "cancelled"},
        "in_progress": {"completed", "cancelled", "blocked"},
        "blocked": {"in_progress", "cancelled"},
        "approved": {"fulfilled", "rejected", "cancelled"},
        "active": {"inactive", "completed", "cancelled", "blocked"},
        "completed": set(),
        "cancelled": set(),
        "rejected": set(),
    }

    def _assert_status_transition(self, entity: Technician, target: str, *, action: str) -> None:
        current = str(getattr(entity, "status", "draft"))
        allowed = self._STATUS_TRANSITIONS.get(current, set())
        if target not in allowed and current != target:
            raise TechnicianValidationError(
                f"Cannot {action} technician from status '{current}' to '{target}'"
            )
        logger.info(
            "technician.status_transition",
            entity_id=str(entity.id),
            from_status=current,
            to_status=target,
            action=action,
        )

    def _set_status(self, entity: Technician, target: str, *, action: str) -> None:
        self._assert_status_transition(entity, target, action=action)
        entity.status = target

    async def _require_entity(
        self, entity_id: UUID, *, tenant_id: UUID | None = None
    ) -> Technician:
        entity = await self._repo.get_by_id(entity_id, tenant_id=tenant_id)
        if entity is None:
            logger.warning("technician.not_found", entity_id=str(entity_id))
            raise TechnicianNotFoundError(entity_id)
        return entity

    def _validate_tenant(self, tenant_id: UUID | None) -> UUID:
        if tenant_id is None:
            raise TechnicianValidationError('X-Tenant-Id is required')
        return tenant_id

    async def get(self, entity_id: UUID, *, tenant_id: UUID | None = None) -> TechnicianRead:
        tenant_id = self._validate_tenant(tenant_id)
        entity = await self._require_entity(entity_id, tenant_id=tenant_id)
        logger.info("technician.service.get", entity_id=str(entity_id))
        return TechnicianRead.model_validate(entity)

    async def list(
        self,
        *,
        tenant_id: UUID | None = None,
        page: int = 1,
        page_size: int = 50,
        order_by: str = "created_at",
        order_dir: str = "desc",
        **filters: Any,
    ) -> tuple[list[TechnicianRead], int]:
        tenant_id = self._validate_tenant(tenant_id)
        if page < 1:
            raise TechnicianValidationError("Page must be >= 1")
        if page_size < 1 or page_size > 200:
            raise TechnicianValidationError("Page size must be between 1 and 200")
        rows, total = await self._repo.list(
            tenant_id=tenant_id,
            page=page,
            page_size=page_size,
            order_by=order_by,
            order_dir=order_dir,
            **filters,
        )
        logger.info(
            "technician.service.list",
            page=page,
            page_size=page_size,
            total=total,
            returned=len(rows),
        )
        return [TechnicianRead.model_validate(r) for r in rows], total

    async def count(self, *, tenant_id: UUID | None = None, **filters: Any) -> int:
        tenant_id = self._validate_tenant(tenant_id)
        return await self._repo.count(tenant_id=tenant_id, **filters)

    async def exists(self, entity_id: UUID, *, tenant_id: UUID | None = None) -> bool:
        tenant_id = self._validate_tenant(tenant_id)
        return await self._repo.exists(entity_id, tenant_id=tenant_id)

    async def create(
        self, data: TechnicianCreate, *, tenant_id: UUID | None = None
    ) -> TechnicianRead:
        tenant_id = self._validate_tenant(tenant_id)
        self._validate_create(data)
        entity = await self._repo.create(data, tenant_id=tenant_id)
        logger.info("technician.service.create", entity_id=str(entity.id), tenant_id=str(tenant_id))
        return TechnicianRead.model_validate(entity)

    async def update(
        self,
        entity_id: UUID,
        data: TechnicianUpdate,
        *,
        tenant_id: UUID | None = None,
    ) -> TechnicianRead:
        tenant_id = self._validate_tenant(tenant_id)
        entity = await self._require_entity(entity_id, tenant_id=tenant_id)
        self._validate_update(entity, data)
        updated = await self._repo.update(entity, data)
        logger.info("technician.service.update", entity_id=str(entity_id))
        return TechnicianRead.model_validate(updated)

    async def delete(
        self, entity_id: UUID, *, tenant_id: UUID | None = None
    ) -> None:
        tenant_id = self._validate_tenant(tenant_id)
        entity = await self._require_entity(entity_id, tenant_id=tenant_id)
        await self._repo.soft_delete(entity)
        logger.info("technician.service.delete", entity_id=str(entity_id))

    async def restore(
        self, entity_id: UUID, *, tenant_id: UUID | None = None
    ) -> TechnicianRead:
        tenant_id = self._validate_tenant(tenant_id)
        entity = await self._repo.get_by_id(entity_id, tenant_id=tenant_id, include_deleted=True)
        if entity is None:
            raise TechnicianNotFoundError(entity_id)
        restored = await self._repo.restore(entity)
        logger.info("technician.service.restore", entity_id=str(entity_id))
        return TechnicianRead.model_validate(restored)

    def _validate_create(self, data: TechnicianCreate) -> None:
        """Domain-specific create validation for Technician."""
        raw = getattr(data, "employee_id", None)
        if raw is not None and not str(raw).strip():
            raise TechnicianValidationError("employee_id is required and cannot be blank")

        if hasattr(data, "status") and data.status is not None:
            if data.status not in {'draft', 'pending', 'active', 'in_progress', 'completed', 'cancelled'}:
                raise TechnicianValidationError(f"Invalid status: {data.status}")

        raw = getattr(data, "status", None)
        if raw is not None and not str(raw).strip():
            raise TechnicianValidationError("status is required and cannot be blank")

    def _validate_update(self, entity: Technician, data: TechnicianUpdate) -> None:
        """Domain-specific update validation for Technician."""
        if data.status is not None and data.status == entity.status:
            raise TechnicianConflictError(f"Status is already {entity.status}")

        new_status = getattr(data, "status", None)
        if new_status is not None and new_status not in {'draft', 'pending', 'active', 'in_progress', 'completed', 'cancelled'}:
            raise TechnicianValidationError(f"Invalid status: {new_status}")

    async def set_status(self, entity_id: UUID, tenant_id: UUID | None, status: str):
        """Update availability status"""
        entity = await self._require_entity(entity_id, tenant_id=tenant_id)
        if not status:
            raise TechnicianValidationError("status is required")
        self._set_status(entity, status, action="set_status")
        logger.info("technician.set_status", entity_id=str(entity.id), status=status)
        await self._repo._session.flush()
        await self._repo._session.refresh(entity)
        return entity

    async def update_location(self, entity_id: UUID, tenant_id: UUID | None, lat: Decimal, lng: Decimal):
        """Record current GPS coordinates"""
        entity = await self._require_entity(entity_id, tenant_id=tenant_id)
        logger.info("technician.update_location.start", entity_id=str(entity.id))
        # Record current GPS coordinates
        logger.info("technician.update_location.complete", entity_id=str(entity.id))
        await self._repo._session.flush()
        await self._repo._session.refresh(entity)
        return entity

    async def calculate_utilization(self, entity_id: UUID, tenant_id: UUID | None, start: date, end: date) -> Technician:
        """Compute utilization for date range"""
        entity = await self._require_entity(entity_id, tenant_id=tenant_id)
        logger.info("technician.calculate_utilization.start", entity_id=str(entity.id))
        # Compute utilization for date range
        logger.info("technician.calculate_utilization.complete", entity_id=str(entity.id))
        await self._repo._session.flush()
        await self._repo._session.refresh(entity)
        return entity
