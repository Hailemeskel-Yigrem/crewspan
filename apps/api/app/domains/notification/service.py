"""Business logic for Notification."""

from __future__ import annotations

from typing import Any
from uuid import UUID

import structlog

from app.domains.notification.exceptions import (
    NotificationConflictError,
    NotificationNotFoundError,
    NotificationValidationError,
)
from app.domains.notification.models import Notification
from app.domains.notification.repository import NotificationRepository
from app.domains.notification.schemas import NotificationCreate, NotificationRead, NotificationUpdate

logger = structlog.get_logger(__name__)


class NotificationService:
    """Orchestrates notification use cases with validation, transitions, and auditing."""

    def __init__(self, repository: NotificationRepository) -> None:
        self._repo = repository

    _STATUS_TRANSITIONS: dict[str, set[str]] = {
        "draft": {"submitted", "cancelled", "active", "pending"},
        "pending": {"approved", "rejected", "cancelled", "active"},
        "submitted": {"in_progress", "assigned", "cancelled"},
        "assigned": {"in_progress", "cancelled"},
        "in_progress": {"completed", "cancelled", "blocked"},
        "blocked": {"in_progress", "cancelled"},
        "approved": {"fulfilled", "rejected", "cancelled"},
        "active": {"inactive", "completed", "cancelled", "blocked"},
        "completed": set(),
        "cancelled": set(),
        "rejected": set(),
    }

    def _assert_status_transition(self, entity: Notification, target: str, *, action: str) -> None:
        current = str(getattr(entity, "status", "draft"))
        allowed = self._STATUS_TRANSITIONS.get(current, set())
        if target not in allowed and current != target:
            raise NotificationValidationError(
                f"Cannot {action} notification from status '{current}' to '{target}'"
            )
        logger.info(
            "notification.status_transition",
            entity_id=str(entity.id),
            from_status=current,
            to_status=target,
            action=action,
        )

    def _set_status(self, entity: Notification, target: str, *, action: str) -> None:
        self._assert_status_transition(entity, target, action=action)
        entity.status = target

    async def _require_entity(
        self, entity_id: UUID, *, tenant_id: UUID | None = None
    ) -> Notification:
        entity = await self._repo.get_by_id(entity_id, tenant_id=tenant_id)
        if entity is None:
            logger.warning("notification.not_found", entity_id=str(entity_id))
            raise NotificationNotFoundError(entity_id)
        return entity

    def _validate_tenant(self, tenant_id: UUID | None) -> UUID:
        if tenant_id is None:
            raise NotificationValidationError('X-Tenant-Id is required')
        return tenant_id

    async def get(self, entity_id: UUID, *, tenant_id: UUID | None = None) -> NotificationRead:
        tenant_id = self._validate_tenant(tenant_id)
        entity = await self._require_entity(entity_id, tenant_id=tenant_id)
        logger.info("notification.service.get", entity_id=str(entity_id))
        return NotificationRead.model_validate(entity)

    async def list(
        self,
        *,
        tenant_id: UUID | None = None,
        page: int = 1,
        page_size: int = 50,
        order_by: str = "created_at",
        order_dir: str = "desc",
        **filters: Any,
    ) -> tuple[list[NotificationRead], int]:
        tenant_id = self._validate_tenant(tenant_id)
        if page < 1:
            raise NotificationValidationError("Page must be >= 1")
        if page_size < 1 or page_size > 200:
            raise NotificationValidationError("Page size must be between 1 and 200")
        rows, total = await self._repo.list(
            tenant_id=tenant_id,
            page=page,
            page_size=page_size,
            order_by=order_by,
            order_dir=order_dir,
            **filters,
        )
        logger.info(
            "notification.service.list",
            page=page,
            page_size=page_size,
            total=total,
            returned=len(rows),
        )
        return [NotificationRead.model_validate(r) for r in rows], total

    async def count(self, *, tenant_id: UUID | None = None, **filters: Any) -> int:
        tenant_id = self._validate_tenant(tenant_id)
        return await self._repo.count(tenant_id=tenant_id, **filters)

    async def exists(self, entity_id: UUID, *, tenant_id: UUID | None = None) -> bool:
        tenant_id = self._validate_tenant(tenant_id)
        return await self._repo.exists(entity_id, tenant_id=tenant_id)

    async def create(
        self, data: NotificationCreate, *, tenant_id: UUID | None = None
    ) -> NotificationRead:
        tenant_id = self._validate_tenant(tenant_id)
        self._validate_create(data)
        entity = await self._repo.create(data, tenant_id=tenant_id)
        logger.info("notification.service.create", entity_id=str(entity.id), tenant_id=str(tenant_id))
        return NotificationRead.model_validate(entity)

    async def update(
        self,
        entity_id: UUID,
        data: NotificationUpdate,
        *,
        tenant_id: UUID | None = None,
    ) -> NotificationRead:
        tenant_id = self._validate_tenant(tenant_id)
        entity = await self._require_entity(entity_id, tenant_id=tenant_id)
        self._validate_update(entity, data)
        updated = await self._repo.update(entity, data)
        logger.info("notification.service.update", entity_id=str(entity_id))
        return NotificationRead.model_validate(updated)

    async def delete(
        self, entity_id: UUID, *, tenant_id: UUID | None = None
    ) -> None:
        tenant_id = self._validate_tenant(tenant_id)
        entity = await self._require_entity(entity_id, tenant_id=tenant_id)
        await self._repo.soft_delete(entity)
        logger.info("notification.service.delete", entity_id=str(entity_id))

    async def restore(
        self, entity_id: UUID, *, tenant_id: UUID | None = None
    ) -> NotificationRead:
        tenant_id = self._validate_tenant(tenant_id)
        entity = await self._repo.get_by_id(entity_id, tenant_id=tenant_id, include_deleted=True)
        if entity is None:
            raise NotificationNotFoundError(entity_id)
        restored = await self._repo.restore(entity)
        logger.info("notification.service.restore", entity_id=str(entity_id))
        return NotificationRead.model_validate(restored)

    def _validate_create(self, data: NotificationCreate) -> None:
        """Domain-specific create validation for Notification."""
        email_val = getattr(data, "recipient_email", None)
        if email_val is not None and "@" not in email_val:
            raise NotificationValidationError("Valid email required for recipient_email")

        raw = getattr(data, "channel", None)
        if raw is not None and not str(raw).strip():
            raise NotificationValidationError("channel is required and cannot be blank")

        raw = getattr(data, "template_key", None)
        if raw is not None and not str(raw).strip():
            raise NotificationValidationError("template_key is required and cannot be blank")

        raw = getattr(data, "body", None)
        if raw is not None and not str(raw).strip():
            raise NotificationValidationError("body is required and cannot be blank")

        if hasattr(data, "status") and getattr(data, "status") is not None:
            if getattr(data, "status") not in {'draft', 'pending', 'active', 'in_progress', 'completed', 'cancelled'}:
                raise NotificationValidationError(f"Invalid status: {getattr(data, 'status')}")

        raw = getattr(data, "status", None)
        if raw is not None and not str(raw).strip():
            raise NotificationValidationError("status is required and cannot be blank")

    def _validate_update(self, entity: Notification, data: NotificationUpdate) -> None:
        """Domain-specific update validation for Notification."""
        if data.status is not None and data.status == entity.status:
            raise NotificationConflictError(f"Status is already {entity.status}")

        new_status = getattr(data, "status", None)
        if new_status is not None and new_status not in {'draft', 'pending', 'active', 'in_progress', 'completed', 'cancelled'}:
            raise NotificationValidationError(f"Invalid status: {new_status}")

    async def mark_sent(self, entity_id: UUID, tenant_id: UUID | None):
        """Record successful delivery"""
        entity = await self._require_entity(entity_id, tenant_id=tenant_id)
        from datetime import datetime, timezone
        entity.status = "sent"
        entity.sent_at = datetime.now(timezone.utc)
        logger.info("notification.mark_sent", entity_id=str(entity.id))
        await self._repo._session.flush()
        await self._repo._session.refresh(entity)
        return entity

    async def mark_failed(self, entity_id: UUID, tenant_id: UUID | None, error: str):
        """Record delivery failure"""
        entity = await self._require_entity(entity_id, tenant_id=tenant_id)
        entity.status = "failed"
        logger.info("notification.mark_failed", entity_id=str(entity.id), error=error)
        await self._repo._session.flush()
        await self._repo._session.refresh(entity)
        return entity

    async def retry(self, entity_id: UUID, tenant_id: UUID | None):
        """Requeue failed notification"""
        entity = await self._require_entity(entity_id, tenant_id=tenant_id)
        if entity.status != "failed":
            raise NotificationValidationError("Only failed notifications may be retried")
        entity.status = "pending"
        logger.info("notification.retry", entity_id=str(entity.id))
        await self._repo._session.flush()
        await self._repo._session.refresh(entity)
        return entity
