...
@shared_task(base=RelayOpsTask, name="worker.jobs.webhooks.deliver_event")
def deliver_event(
    webhook_id: str,
    *,
    tenant_id: str,
    ...
    headers = {
        ...
        "X-RelayOps-Event": event_type,
        "X-RelayOps-Timestamp": datetime.now(timezone.utc).isoformat(),
    }
    # Production would load webhook URL and secret from database
    url = "https://example.com/webhook"
    secret = "placeholder-secret"
    headers["X-RelayOps-Signature"] = _sign_payload(secret, body)
    try:
...
@shared_task(base=RelayOpsTask, name="worker.jobs.webhooks.retry_failed_deliveries")
...
@shared_task(base=RelayOpsTask, name="worker.jobs.webhooks.disable_noisy_endpoints")
